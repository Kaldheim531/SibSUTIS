...
double Newton(double x, int n, double x_arr[], double y_arr[]) {
    printf("Интерполяционный полином Ньютона:\n");
    printf("N(x) = ");
    
    double sum = y_arr[0];
    printf("%.6f", y_arr[0]); // Первый член полинома
    
    for(int i = 1; i < n; ++i) {
        printf(" + "); // Подготовка к выводу следующего члена
        
        // Вычисление разделённой разности i-го порядка
        double F = 0;
        printf("( ");
        for(int j = 0; j <= i; ++j) {
            double den = 1;
            ...
            // Вывод компоненты разделённой разности
            printf("%.6f/(", y_arr[j]);
            for(int k = 0; k <= i; ++k) {
                if(k != j) {
                    printf("(%.6f-%.6f)", x_arr[j], x_arr[k]);
                    if(k < i && (k != i-1 || j != i)) printf("*");
                }
            }
            printf(")");
            if(j < i) printf(" + ");
